...
class Vec3{
    ...
    Vec3 refract(Vec3& n, float etai_over_etat) {
        float cos_theta = sycl::fmin(-this->dot(n), 1.0f);
        Vec3 r_out_perp =  (*this + n*cos_theta)*(etai_over_etat);
        Vec3 r_out_parallel = -n*sycl::sqrt(sycl::fabs(1.0f - r_out_perp.length()*r_out_perp.length()));
    ...
    bool near_zero() const {
        ...
        auto s = 1e-8f;
        return (sycl::fabs(x) < s) && (sycl::fabs(y) < s) && (sycl::fabs(z) < s);
    }

    template <class rng_t>
    Vec3 random_unit_vector(rng_t rng, xorwow_state_t* local_rand_state){
        while(true){
            auto p = Vec3(rand_uniform(rng, local_rand_state), rand_uniform(rng, local_rand_state), rand_uniform(rng, local_rand_state));
            auto lensq = p.length()*p.length();
            if (1e-160f < lensq && lensq <= 1.0f){
                return p/sycl::sqrt(lensq);
            }
        }
    }

    template <class rng_t>
    Vec3 random_on_hemisphere(Vec3 & normal, rng_t rng, xorwow_state_t* local_rand_state){
        Vec3 on_unit_sphere = normal.random_unit_vector(rng, &local_rand_state);
